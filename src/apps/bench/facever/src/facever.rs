...
fn client(args: &[&str]) {
    ...
    reply_gate.activate();
    let mut sgate = SendGate::new_named("req").expect("Unable to create named SendGate req");
...
fn storage(args: &[&str]) {
    ...
    let gpu_sgate = SendGate::new_named("gpu").expect("Unable to create named SendGate gpu");
